...
斜杠命令使用固定规则解析，不经过大模型。飞书把 GitHub URL 展开成 `GitHub - owner/repository` 链接预览时也可以直接绑定，无需重新复制纯文本地址。
...
# 单次方案的最大模型决策步数。大型项目建议先使用 300 步
AGENT_MAX_STEPS=300

# 单次调查最长耗时（秒）；步数或耗时任一耗尽都会生成未完成方案
...
提高 `AGENT_MAX_STEPS` 有助于调查大型项目，但会增加延迟和 Token 消耗。程序会在剩余约 20% 预算时提醒模型优先补齐关键证据并收尾。达到步数或 `AGENT_MAX_SECONDS` 耗时限制时，已经记录且通过位置校验的内容会生成明确标注的“未完成方案”；未覆盖需求进入待调查列表，不会用无证据的推测补齐结果。模型网络失败或持续返回非法结构仍会明确失败。
